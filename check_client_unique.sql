...
counts AS (
    SELECT
        (SELECT COUNT(*) FROM normalized n2 WHERE n2.email_clean = n1.email_clean) AS email_count,
        (SELECT COUNT(*) FROM normalized n3 WHERE n3.phone_clean = n1.phone_clean) AS phone_count,
        n1.*
    FROM normalized n1
)
SELECT
...
JOIN counts c ON c.client_id = cu.client_id
